...
async function testBackend() {
    try {
        ...
        // 1. Login Admin
        const adminRes = await axios_1.default.post(`${API_URL}/auth/login`, {
        ...
        // 2. Login Manager
        const managerRes = await axios_1.default.post(`${API_URL}/auth/login`, {
        ...
        // 3. Login Employee
        const employeeRes = await axios_1.default.post(`${API_URL}/auth/login`, {
        ...
        // 4. Get Divisions
        const divisions = await axios_1.default.get(`${API_URL}/admin/divisions`, {
            headers: { Authorization: `Bearer ${adminToken}` }
        });
        console.log('Divisions:', divisions.data.length);
        console.log('\n--- Testing Manager Routes ---');
        // 5. Create Goal
        const goalRes = await axios_1.default.post(`${API_URL}/goals`, {
        ...
        const goalsRes = await axios_1.default.get(`${API_URL}/goals`, {
            headers: { Authorization: `Bearer ${managerToken}` }
        });
        console.log('Manager Goals:', goalsRes.data.length);
        console.log('\n--- Testing Employee Routes ---');
        // 7. Get Goals as Employee
        const empGoalsRes = await axios_1.default.get(`${API_URL}/goals`, {
            headers: { Authorization: `Bearer ${employeeToken}` }
        });
        console.log('Employee Goals:', empGoalsRes.data.length);
        // 8. Create Task
        const taskRes = await axios_1.default.post(`${API_URL}/tasks`, {
        ...
        const tasksRes = await axios_1.default.get(`${API_URL}/tasks`, {
            headers: { Authorization: `Bearer ${employeeToken}` }
        });
...
testBackend();
//# sourceMappingURL=verify_backend.js.map